<template>
  <div class="row">
    <div class="col">
      ...
      <button @click="activateBoost" :disabled="boostActive || boostCooldown" class="btn btn-warning mt-3">
        Double Cookie Boost
      </button>
      <p v-if="boostCooldown">Boost on cooldown...</p>
    </div>
    ...
    created() {
        setInterval(() => {
            for (const upgrade in this.upgrades) {
                const cps = this.boostActive ? this.upgrades[upgrade].cps * 2 : this.upgrades[upgrade].cps;
                this.cookies = (parseFloat(this.cookies) + cps * this.upgrades[upgrade].count).toFixed(1);
            }
        }, 1000);
    },
    data() {
